...
在之后的结构式中，我们不会特别注明α碳的手性，但会把主链用灰色弱化。如果把结构式转写成化学式，则为 $\ce{H2N-CH(-R)-COOH}$，或 $\ce{R-C2H4O2N}$。
...
脯氨酸是氨基酸中唯一一种仲胺，也就是 $\ce{R2NH}$，而不是 $\ce{RNH2}$。可以把它看作是先连一个丙基，然后环化形成的吡咯烷。
...
- 缩写：Lys (K)
- 取代基：4-胺基丁基    $\ce{-CH2-CH2-CH2-CH2-NH2}$
...
- 缩写：Arg (R)
- 取代基：3-胍基丙基    $\ce{-CH2-CH2-CH2-NH-C(=NH)-NH2}$
